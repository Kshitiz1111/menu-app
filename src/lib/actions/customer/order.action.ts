...
export const saveOrdersByRestaurant = async(order:any, restaurant_id:string)=>{
   ...
   try {
      ...
      await client.query(`
         ...
         BEGIN
            ...
            AND table_name = '${restaurant_id}_orders'
            ) THEN
            CREATE TABLE ${restaurant_id}_orders (
      ...
      const checkOrderExists = await client.query(
         `SELECT 1 FROM ${restaurant_id}_orders WHERE purchase_order_id = $1 LIMIT 1;`,
         [order.purchase_order_id]
      );
      if (checkOrderExists.rows.length > 0) {
         console.error("Order with this purchase_order_id already exists.");
         client.release();
         return { success: false, message:  `Order with ${checkOrderExists.rows[0].purchase_order_id} this purchase_order_id already exists.` };
      }
      
      // Insert order into 'orders' table
      const result:any = await client.query(
         `INSERT INTO ${restaurant_id}_orders (purchase_order_id, no_of_item, total_price, compensation, status) 
      ...
      await client.query(`
         ...
         BEGIN
            ...
            SELECT FROM information_schema.tables 
            WHERE table_schema = 'public' 
            AND table_name = '${restaurant_id}_order_details'
            ) THEN
            CREATE TABLE ${restaurant_id}_order_details (
               ...
               customer_note TEXT,
               order_id INTEGER REFERENCES ${restaurant_id}_orders(order_id)
            );
            END IF;
         END $$;
      `);
      ...
      for (const detail of order.details) {
         await client.query(
            `INSERT INTO ${restaurant_id}_order_details (dish_id, quantity, price, customized_ingredients, customer_note, order_id) 
            VALUES ($1, $2, $3, $4, $5, $6);`,
